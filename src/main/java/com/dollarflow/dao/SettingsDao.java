...
public class SettingsDao {
    ...
    public Settings get() {
        String sql = "SELECT company_name, company_address, cgst_rate, sgst_rate, starting_bill_no "
                + "FROM settings WHERE id = 1";
        try (Statement st = Database.get().createStatement();
             ResultSet rs = st.executeQuery(sql)) {
            if (!rs.next()) {
                return new Settings("", "", BigDecimal.valueOf(9), BigDecimal.valueOf(9), 10_000);
            }
            return new Settings(
    ...
    public void save(Settings settings) {
        String sql = """
                UPDATE settings
                SET company_name = ?, company_address = ?, cgst_rate = ?, sgst_rate = ?, starting_bill_no = ?
                WHERE id = 1
                """;
        try (PreparedStatement ps = Database.get().prepareStatement(sql)) {
            ...
            ps.setInt(5, settings.startingBillNo());
            ps.executeUpdate();
        } catch (SQLException e) {
